...
import { useCreateResource } from '../hooks/resources/useCreateResource.js';
...
export const CreateApiResourcePage = () => {
    ...
    const EndpointSchema = z.object({
        ...
        field_map: z.array(z.object({ 
            ...
            value: z.string()
                ...
                .regex(/^[a-zA-Z0-9._\[\]]+$/, 'Field value must use dot notation (e.g., title, volumeInfo.title, items[0].name)')
    ...
    const handleSubmit = async (e) => {
        ...
        setFormErrors({});
        try {
            // First validate the form data as-is
            ...
            // Append API resource configuration using bracket notation (like Redirect page)
            ...
            formDataToSend.append('api_resource[sync_status]', formData.api_resource.sync_status);
            formDataToSend.append('api_resource[auth_location]', formData.api_resource.auth_location);
            formDataToSend.append('api_resource[auth_type]', formData.api_resource.auth_type);
            formDataToSend.append('api_resource[timeout]', String(formData.api_resource.timeout));
            formDataToSend.append('api_resource[retry_attempts]', String(formData.api_resource.retry_attempts));
            ...
            transformedEndpoints.forEach((ep, i) => {
                ...
                formDataToSend.append(`api_resource[endpoints][${i}][method]`, ep.method);
                ...
                formDataToSend.append(`api_resource[endpoints][${i}][status]`, ep.status);
                formDataToSend.append(`api_resource[endpoints][${i}][sync_status]`, ep.sync_status);
            ...
            console.log("Submitting FormData with bracket notation");
            await mutateAsync(formDataToSend);
            success('API resource created successfully');
            ...
            if (err instanceof z.ZodError) {
                ...
                err.issues.forEach((issue) => {
                    ...
                    if (!friendly[path]) friendly[path] = issue.message;
                });
            ...
            <div className="w-full">
                <Breadcrumb
                    items={[
                        ...
                        { label: 'Choose Type of Resource', onClick: () => navigate('/admin/resources/choose') },
                        { label: 'Create API Resource' }
                    ]}
                />
                        ...
                        <div className="flex flex-col">
                            ...
                            <div>
                                ...
                                <p className="text-blue-100 mt-1">
                                    Create a new API resource with endpoints and data paths
                                </p>
                            </div>
                        </div>
                    </div>
